<!-- mdformat global-off -->
# Pretrain deepseekv3-671b-bf16-gbs4096-gpus256 workloads on a4x GKE Node pools with Megatron-Bridge

This recipe outlines the steps for running a deepseekv3-671b pretraining
...
- Node Configuration: 64 nodes (4 GPUs per node, 256 GPUs total).
...
export RECIPE_ROOT=$REPO_ROOT/training/a4x/deepseekv3-671b/megatron-bridge-pretraining-gke/64node-BF16-GBS4096/recipe
cd $RECIPE_ROOT
```

### Configure and submit a pretraining job

#### Using 64 nodes (256 gpus) bf16 precision

To execute the job with the default settings, run the following command from your client:

```bash
cd $RECIPE_ROOT
export WORKLOAD_NAME=$USER-deepseekv3-671b-64node-bf16-gbs4096
...
```
kubectl get pods | grep $USER-qwen3-235b-64node-bf16-gbs4096
```

Replace the following:

- JOB_NAME_PREFIX - your job name prefix. For example $USER-qwen3-235b-64node-bf16-gbs4096.
...
For example: `$USER-deepseekv3-671b-64node-bf16-gbs4096-workload-0-0-s9zrv`.
...
```bash
helm uninstall $USER-deepseekv3-671b-64node-bf16-gbs4096
```